...
def decode_image(file_location,qrimgname2):
    ...
    for i in range(x_size):
        for j in range(y_size):
            if bin(red_channel.getpixel((i, j)))[-1] == '0':
                pixels[i, j] = (1000, 1000, 1000)
            else:
                pixels[i, j] = (0, 0, 0)
    decoded_image.save("static/decodeimage/"+ qrimgname2)
...
def encode_image(text_to_encode, template_image,qrimgname):
    ...
    encoded_image.save("static/encodeimage/" + qrimgname)
